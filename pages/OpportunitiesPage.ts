...
export class OpportunitiesPage {
  ...
  async assertPageLoaded(): Promise<void> {
    ...
    await expect(this.page.locator('.slds-page-header').first()).toBeVisible({ timeout: 20000 });
  }

  async assertOpportunityName(name: string): Promise<void> {
    await expect(this.page.getByText(name, { exact: false }).first()).toBeVisible({ timeout: 20000 });
  }
}
